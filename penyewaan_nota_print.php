      ...
      <ul class="sidebar-menu" data-widget="tree">
        ...
        <li class="treeview">
          ...
          <ul class="treeview-menu">
            ...
            <li class="active"><a href="penyewaan.php"><img src="img/transaksi.png" style="width: 15px">Transaksi Penyewaan</a></li>
          </ul>
        </li>
        <li class="treeview">
          <a href="#">
        ...
        <li class="active"><a href="grafik.php"><i class="fa fa-line-chart"></i> <span>Grafik</span></a></li>
      </ul>
    </section>
    <!-- /.sidebar -->
  </aside>
...
<?php
  $id=$_GET['id'];
  $record=mysqli_query($kon, "SELECT * FROM transaksi_penyewaan WHERE id='$id'");
  $data=mysqli_fetch_assoc($record);
  $id_member=$data['id_member'];
  $record=mysqli_query($kon, "SELECT * FROM member WHERE id='$id_member'");
  $data2=mysqli_fetch_assoc($record);
  $rp_total_harga=$data['total_harga'];
?>
<table border="1" style="" id="" class="table table-hover table-bordered">
  <thead>
      <tr>
        <th colspan="2">No Transaksi: <?php echo $data['id'].'/'.$data['tgl_sewa'] ?></td>
      </tr>
      <tr>
        <td colspan="2">Nama: <?php echo $data2['nama_lengkap']?></td>
      </tr>
      <tr>
        <td>Tanggal Penyewaan: <?php echo $data['tgl_sewa'] ?></td>
        <td>Tanggal Pengembalian: <?php echo $data['tgl_kembali'] ?></td>
      </tr>
      <tr>
        <td colspan="2">Total harga yang harus dibayarkan: <?php echo rupiah($rp_total_harga) ?></td>
      </tr>
  </thead>
  <!-- <?php  
      //include('koneksi.php');
      //$query = "SELECT * FROM transaksi_kg ORDER BY id ASC";
        //$result = mysqli_query($kon, $query);
        //$no = 1;
        //while($record = mysqli_fetch_assoc($result)){
          //echo "<form action=\"kilogram_edit_delete.php\">";
    ?> -->
</form>
</table>
  <script>
